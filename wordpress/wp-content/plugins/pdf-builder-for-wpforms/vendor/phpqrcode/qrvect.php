 ...
 */
    ...
    class QRvect {
        ...
        private static function vectSVG($frame, $pixelPerPoint = 4, $outerFrame = 4, $back_color = 0xFFFFFF, $fore_color = 0x000000) 
        {
         ...
         /*   $output .=
            '<defs>'."\n".
            '<rect id="p" width="'.($pixelPerPoint*2).'" height="'.($pixelPerPoint*2).'" />'."\n".
            '</defs>'."\n";
            ...
            for($i=0; $i<$h; $i++) {
                for($j=0; $j<$w; $j++) {
                    if( $frame[$i][$j] == '1') {
                        $y = ($i + $outerFrame) * $pixelPerPoint;
                        $x = ($j + $outerFrame) * $pixelPerPoint;
                        $output .= '<rect fill="#'.str_pad(dechex($fore_color), 6, "0", STR_PAD_LEFT).'" x="'.$x.'" y="'.$y.'" width="'.($pixelPerPoint).'" height="'.($pixelPerPoint).'"  />'."\n";
                    }
                }
            }
            $output .=
