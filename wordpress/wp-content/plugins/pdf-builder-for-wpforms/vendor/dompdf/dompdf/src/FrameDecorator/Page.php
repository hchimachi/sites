...
class Page extends AbstractFrameDecorator
{
    ...
    function check_page_break(Frame $frame)
    {
        ...
        if($frame->get_frame()->get_node()->nodeType == XML_ELEMENT_NODE&&$frame->get_frame()->get_node()->getAttribute('class')=='footer')
        {
            $previousSection=$frame->get_prev_sibling();
            while($previousSection!=null)
            {
                if($previousSection->get_node()->nodeType==XML_ELEMENT_NODE&&$previousSection->get_frame()->get_node()->getAttribute('class')=='content')
                    break;

                $previousSection=$previousSection->get_prev_sibling();
            }

            if($previousSection==null)
                return false;

            $previousSectionLastChild=$previousSection->get_last_child();
            if($previousSectionLastChild==null)
                return false;
            if($previousSectionLastChild->get_position('y')+$previousSectionLastChild->get_margin_height()<$frame->get_position('y'))
            {
                return false;
            }
