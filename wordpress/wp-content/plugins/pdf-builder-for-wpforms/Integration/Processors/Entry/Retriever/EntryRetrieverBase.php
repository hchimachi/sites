...
abstract class EntryRetrieverBase
{
    ...
    public function InitializeFromOriginalEntryId($id)
    {
        ...
        if($entryId===false)
            return false;
        return $this->InitializeFromEntryId($id);
    }
    ...
    {
        global $wpdb;
        $recordData= $wpdb->get_results($wpdb->prepare('select fields,form.fields original_fields, entry,seq_num,date,raw from '.$this->Loader->RECORDS_TABLE.' record join '.$this->Loader->FormConfigTable.' form on form.id=record.form_id where record.id=%d' ,$entryId));
     ...
     */
    public function InitializeByEntryItems($entryItems,$raw=null,$originalFields)
    {
