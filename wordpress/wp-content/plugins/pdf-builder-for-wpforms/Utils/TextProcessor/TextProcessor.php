...
namespace rednaoformpdfbuilder\Utils\TextProcessor;

use rednaoformpdfbuilder\core\Loader;
use rednaoformpdfbuilder\Utils\TextProcessor\Font\Cache;
use rednaoformpdfbuilder\Utils\TextProcessor\Font\FontCache;
use rednaoformpdfbuilder\Utils\TextProcessor\Font\FontFileFinder;
use rednaoformpdfbuilder\Utils\TextProcessor\Font\MetricsGenerator;
use rednaoformpdfbuilder\Utils\TextProcessor\Font\Otl;
...
class TextProcessor
{
    ...
    private function __construct()
    {
        ...
        $this->fontCache = new FontCache(new Cache(self::$Loader->DIR . 'vendor/dompdf/dompdf/lib/fonts'));
        $this->fontFileFinder = new FontFileFinder(SELF::$Loader->DIR . 'vendor/dompdf/dompdf/lib/fonts');
    ...
    public function Process($text)
    {
        ...
        $this->_saveTextBuffer($text);
        $chuckorder=[0];

        $this->otl->bidiPrepare($this->textbuffer,'ltr');
        $data=array($this->textbuffer[0][18]);
        $content=array($text);
        $this->otl->bidiReorder($chuckorder, $content,$data , 'ltf');
        return $content[0];
