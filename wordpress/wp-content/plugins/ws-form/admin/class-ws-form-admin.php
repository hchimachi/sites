<?php
	...
	class WS_Form_Admin {
		...
		public function enqueue_styles($hook) {
			...
			// Dequeue styles added by other plugins (they should only be enqueing on their admin pages)
		...
		public function enqueue_scripts($hook) {
			...
			// Get form ID
			$this->form_id = intval(WS_Form_Common::get_query_var('id', 0));
			...
			switch($hook) {
				...
				case $this->hook_suffix_form_edit :
					...
					if($this->intro) {
						...
						wp_enqueue_script($this->plugin_name . '-intro', sprintf('%sadmin/js/external/intro%s.js', WS_FORM_PLUGIN_DIR_URL, $min), array('jquery'), $this->version, true);
					}

					// Select2 (Check made because WooCommerce enqueues this)
					wp_enqueue_script($this->plugin_name . '-select2', sprintf('%sshared/js/external/select2.full%s.js', WS_FORM_PLUGIN_DIR_URL, $min), array('jquery'), $this->version, false);
		...
		$(function() {
			...
			$('[data-action="wsf-feedback-submit"]').click(function() {
				...
				$.ajax({
					...
					url: '<?php echo esc_html(WS_Form_Common::get_api_path('helper/deactivate-feedback-submit/')); ?>',
					data: {
...
<?php
		...
		public function admin_menu() {
			...
			$this->hook_suffix_form_upgrade = add_submenu_page(
				...
				__('<span style="color: #418FDE;">Upgrade to PRO</span>', 'ws-form'),
				'manage_options_wsform',
			...
			$this->hook_suffix_form_add_ons = add_submenu_page(
				...
				$this->plugin_name,
				__('Add-Ons', 'ws-form'),
				__('<span style="color: #418FDE;">Add-Ons</span>', 'ws-form'),
				'manage_options_wsform',
		...
		public function screen_settings_submit($current, $screen){
			...
			if($screen->id === $this->hook_suffix_form_submit) {
				...
				$current .= '<fieldset class="screen-options"><legend>Submissions</legend>';
			...
			$current .= sprintf('<input type="hidden" name="id" value="%u" />', intval($this->form_id));
		...
		public function ws_form_default_hidden_columns($hidden, $screen) {
			...
			$form_id = intval(WS_Form_Common::get_query_var('id'));
		...
		public function block_render($attributes, $content) {
			...
			$return_html = do_shortcode(sprintf('[%s id="%u"%s]', WS_FORM_SHORTCODE, $form_id, $form_element_id));
		...
		public function admin_init() {
			...
			switch($page) {
				...
				case 'ws-form-submit' :
					...
					$submit_id = intval(WS_Form_Common::get_query_var_nonce('submit_id', ''));
		...
		public function admin_bar_menu($wp_admin_bar) {
			...
			$wp_admin_bar->add_node(
				...
				array(
					...
					'title' => sprintf('<span class="ab-icon">%s</span><span class="ab-label">%s</span>', WS_Form_Common::get_admin_icon('#a0a5aa', false), __('WS Form', 'ws-form')),
