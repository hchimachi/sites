(function($) {
	...
	$.WS_Form.prototype.object_upload_json = function(files, upload_json_window_obj, obj, success_callback, error_callback, show_confirm) {
		...
		// Build URL
		var url = ws_form_settings.url + object + '/' + object_id + '/upload/json';
	...
	$.WS_Form.prototype.form_download = function() {
		...
		// Build downloader
		var downloader_html = '<form id="wsf-form-downloader" action="' + ws_form_settings.url + 'form/' + this.form_id + '/download/json" method="post">';
...
//		this.sidebar_reset();
	...
	$.WS_Form.prototype.object_download = function(obj) {
		...
		// Build downloader
		var downloader_html = '<form id="wsf-object-downloader" action="' + ws_form_settings.url + object + '/' + object_id + '/download/json" method="post">';
	...
	$.WS_Form.prototype.template_download = function(template_id) {
		...
		// Build downloader
		var downloader_html = '<form id="wsf-template-downloader" action="' + ws_form_settings.url + 'template/download/json" method="post">';
...
//					}
	...
	$.WS_Form.prototype.section_upload_ajax = function(form_data, status_bar, obj, success_callback, error_callback) {
		...
		var url = ws_form_settings.url + 'template/upload/json';
		...
		$('select[data-wsf-select2][data-meta-key="' + data_meta_key + '"]', obj).each(function() {
			...
			var config = {
				...
				ajax: {
					...
					// AJAX URL
					url: ws_form_settings.url + 'select2/' + select_ajax_method_search,
	...
	$.WS_Form.prototype.data_grid_upload_csv_ajax = function(object, object_id, form_data, status_bar, obj, success_callback) {
		...
		var url = ws_form_settings.url + object + '/' + object_id + '/upload/csv';
	...
	$.WS_Form.prototype.submit_export_do = function(data, page, hash, records_total) {
		...
		var url = ws_form_settings.url + 'submit/export/';
			...
			success: function(response) {
				...
				if(typeof(response.complete) !== 'undefined') {
					...
					if(complete) {
						...
						location.href = ws_form_settings.url + 'submit/export/' + hash + '/?_wpnonce=' + ws_form_settings.x_wp_nonce;
	...
	$.WS_Form.prototype.api_call_process = function(ajax_path, method, params, success_callback, error_callback, checksum_request, bypass_loader, bypass_form_processing) {
		...
		// Make AJAX request
		var url = ws_form_settings.url + ajax_path;
