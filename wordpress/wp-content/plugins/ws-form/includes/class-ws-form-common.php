...
<?php
		...
		public static function get_form_object_from_post_file() {
			...
			// Check file extension
			$ext = strtolower(pathinfo($file_name, PATHINFO_EXTENSION));
			if($ext !== 'json') { self::throw_error(__('Unsupported file extension', 'ws-form') . ': ' . $ext); }

			// Check file format
			if(!file_exists($file_tmp_name)) { self::throw_error(__('Unable to read uploaded file', 'ws-form') . ': ' . $ext); }
		...
		public static function get_form_object_from_json($form_json, $checksum_check = false) {
			...
			if(
				...
				self::throw_error(__('JSON corrupt (Not a WS Form JSON file)', 'ws-form'));
			}
